...
def match_template(cur_screenshot,template_path,threshold = 0.05,return_center = True
					,print_debug = True,scope = None,except_locs = None):
	global templates

	if(print_debug):
		print("ImageProcessor: start to match cur_screenshot by "+template_path)

	if(print_debug and except_locs != None):
		print("ImageProcessor: except_locs: "+str(except_locs))

	target = cur_screenshot
	
	if template_path not in templates:
		template = cv2.imread(template_path)
		templates[template_path] = template
	else:
		template = templates.get(template_path)
