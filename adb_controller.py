...
def screenshot_obj():
	...
	screenshot = np.asarray(bytearray(output), dtype=np.uint8)
...
def check_if_accidents(accidents):
	...
	cur_screenshot = screenshot_obj()

	for index in range(0,len(accidents["paths"])):
		match_loc = image_processor.match_template(
			cur_screenshot,accidents["paths"][index],accidents["thresholds"][index],True,print_debug = False)
...
def wait_till_match_any(template_paths,thresholds,return_center,max_time,step_time,accidents=None,scope = None,except_locs = None):
	...
	while(True):
		...
		for index in range(0,len(template_paths)):
			match_loc = image_processor.match_template(
				cur_screenshot,template_paths[index],thresholds[index],return_center,scope = scope,except_locs = except_locs)
			if(match_loc != None):
				return match_loc
...
def wait_while_match(template_paths,thresholds,max_time,step_time,accidents = None,scope = None,except_locs = None):
	...
	while(True):
		...
		for i in range(0,len(template_paths)):
			match_loc = image_processor.match_template(
				cur_screenshot,template_paths[i],thresholds[i],True,scope = scope,except_locs=except_locs)
			if(match_loc != None):
				break
